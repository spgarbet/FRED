...
#    fig_title = args.title
    ...
    result = fred_convert_syntheco_pop()
    sys.exit(result)


def fred_convert_syntheco_pop() -> Literal[0, 2]:
    '''
    ...
    _get_fips_from_lat_lon(40.4379396, -79.9919442)
    _get_fips_from_lat_lon('error', -79.9919442)
...
def _get_fips_from_lat_lon(lat: float, lon: float) -> str:
    '''
    Calls out to the census API to get a FIPS code given a latitude and longitude
    ...
    -------
    str
        The eleven-digit FIPS code used by FRED: stcotrbg
        st = state, co = county, tr = tr, bg = block group
        Or ERR_MSG if there is a problem
    '''
    api_url = 'https://geo.fcc.gov/api/census/block/find?latitude={0}'.format(lat)
    api_url += '&longitude={0}'.format(lon)
    api_url += '&censusYear=2020&showall=false&format=json'
    response = requests.get(api_url)
    if response.status_code == 200:
        try:
            fips_11_digit = response.json()['Block']['FIPS'][:11]
            print(response.json()['Block']['FIPS'])
            print(fips_11_digit)
            return fips_11_digit
        except (KeyError) as e:
            logging.error(logging.error('There is no key {0} in the response JSON'.format(e)))
            return constants.ERR_MSG
    elif response.status_code == 400:
        try:
            print(response.json()['status4Message'])
            logging.error(response.json()['statusMessage'])
            return constants.ERR_MSG
        except (KeyError) as e:
            logging.error('There is no key {0} in the response JSON'.format(e))
            return constants.ERR_MSG
    else:
        logging.error('Unhandled status code returned from API: {0}'.format(response.status_code))
        return constants.ERR_MSG
