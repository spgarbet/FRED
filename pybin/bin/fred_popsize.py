...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=
        'Prints the size of the synthetic population used in a given Job.',
        epilog=usage,
    ...
    result = fred_popsize(fred_key)
    sys.exit(result)


def fred_popsize(fred_key: str) -> Literal[0, 2]:
    '''
    Prints the value stored in the POPSIZE META file for the FRED Job correspoinding
    ...
    # Get popsize
    try:
        popsize = fredutil.get_meta_file_data(fred_job_meta_dir_str,
                                              'POPSIZE',
                                              type=int)
        print(popsize)
        return constants.EXT_CD_NRM
