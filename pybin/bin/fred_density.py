...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=
        'Prints the density of the synthetic population used in a given Job.',
        epilog=usage,
    ...
    result = fred_density(fred_key)
    sys.exit(result)


def fred_density(fred_key: str) -> Literal[0, 2]:
    '''
    Prints the value stored in the DENSITY META file for the FRED Job correspoinding
    to the specified Job key.
    ...
    # Get density
    try:
        density = fredutil.get_meta_file_data(fred_job_meta_dir_str,
                                              'DENSITY',
                                              type=float)
        print(density)
        return constants.EXT_CD_NRM
    except (frederr.FredFileSizeError, ValueError) as e:
        logging.error(e)
        return constants.EXT_CD_ERR
