...
def main():
    ...
    usage = '''
    Note: this does NOT actually change to the directory.
    
    Typical usage example:
    
    $ fred_cd.py mykey
    FRED_RESULTS/JOB/{id}/OUT
    '''
    ...
    result = fred_cd(fred_key)
    sys.exit(result)


def fred_cd(fred_key: str) -> Literal[0, 2]:
    '''
    Prints the full path to the OUT directory of the Job associated with the key to stdout.
    ...
    # Print OUT directory if key was found
    if fred_job_out_dir_str == None:
        print(f'Key [{fred_key}] was not found.')
    else:
        print(fred_job_out_dir_str)
